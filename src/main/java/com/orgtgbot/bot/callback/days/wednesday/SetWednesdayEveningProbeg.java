...
public class SetWednesdayEveningProbeg implements CallbackHandler {
    ...
    public Buttons callbackData() {
        return Buttons.SET_EVENING_WEDNESDAY_KM;
    }

    @Override
    public void handle(CallbackQuery callbackQuery) throws Exception {
        String result = probegService.getEveningKm(callbackData());

        sender.editMarkup(
                callbackQuery.getMessage().getChatId(),
                callbackQuery.getMessage().getMessageId(),
                Buttons.SET_EVENING_WEDNESDAY_KM.getName() + result + " km.",
                KeyboardFactory.probegBack(Buttons.SET_EVENING_WEDNESDAY_KM)
        );
    }
}
